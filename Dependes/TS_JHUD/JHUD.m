...
-(void)showAtView:(UIView *)view;
{
    ...
    for (int index = 1; index<=12; index++) {
        NSString * imageName = [NSString stringWithFormat:@"jiazai_%d.png",index];
        UIImage *image = [UIImage imageNamed:imageName];
        [images addObject:image];
    }
    self.indicatorViewSize = CGSizeMake(100, 100);
    self.customAnimationImages = images;
//    self.messageLabel.font = PFR13Font;
//    self.messageLabel.textColor = C_999999;
...
-(void)showFailureAtView:(UIView *)view{
    self.indicatorViewSize = CGSizeMake(100, 100);
    self.messageLabel.text = @"网络出错啦，请重新加载~";
//    self.messageLabel.font = PFR13Font;
//    self.messageLabel.textColor = C_999999;
    [self.refreshButton setTitle:@"刷新" forState:UIControlStateNormal];
    self.customImage = [UIImage imageNamed:@"net_error"];
    [self showAtView:view hudType:JHUDLoadingTypeFailure];
}

-(void)showFailureAtView:(UIView *)view message:(NSString *)message{
    self.indicatorViewSize = CGSizeMake(100, 100);
    self.messageLabel.text = message;
//    self.messageLabel.font = PFR13Font;
//    self.messageLabel.textColor = C_999999;
    self.customImage = [UIImage imageNamed:@"net_error"];
    [self.refreshButton setTitle:@"再试一次" forState:UIControlStateNormal];
    [self showAtView:view hudType:JHUDLoadingTypeFailure];
}
...
-(UIButton *)refreshButton
{
   ...
   // [self.refreshButton setTitleColor:C_3B3B3B forState:UIControlStateNormal];
  ...
  //  [self.refreshButton hairlineWidth:C_3B3B3B];
...
-(void)updateConstraints
{
    ...
    [self addConstarintWithTopView:self.indicatorView toBottomView:self.messageLabel constarint:6];
    ...
    [self addConstarintWithTopView:self.messageLabel toBottomView:self.refreshButton constarint:14];
    [self.refreshButton addConstraintWidth:84 height:32];
